...
```{r}
# Ensure `resUSDC` and `collateral` are integers
merged_data$resUSDC <- as.integer(merged_data$resUSDC)
merged_data$collateral <- as.integer(merged_data$collateral)
...
# Create the collateral bar plot
...
combined_plot <- subplot(
  ...
  nrows = 6,
  shareX = TRUE,
  heights = c(0.2, 0.2, 0.15, 0.15, 0.1, 0.1)  # Adjust height ratios
) %>%
  layout(
    title = "Pool ALM",
    xaxis = list(
      ...
      rangeslider = list(
        visible = TRUE,
        thickness = 0.02  # Thickness of the range slider
      )
    ),
    ...
    yaxis6 = list(title = "Collateral Amount") # Y-axis for the collateral chart
  )
