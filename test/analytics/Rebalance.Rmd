...
# Create the supplied line chart
plot6 <- ggplot(merged_data) +
  geom_line(aes(x = bN, y = supplied), color = "darkred", linewidth = 1) +
  labs(title = "Supplied Chart", y = "Supplied Amount") +
  theme_minimal()
...
combined_plot <- subplot(
  ...
  nrows = 6,
  shareX = TRUE,
  heights = c(0.3, 0.2, 0.15, 0.15, 0.1, 0.1)  # Adjust height ratios
) %>%
  layout(
    title = "Pool ALM",
    xaxis = list(
      ...
      rangeslider = list(
        visible = TRUE,
        thickness = 0.01  # Thickness of the range slider
      )
    ),
    yaxis = list(title = "TVL"),           # Y-axis for the TVL chart
    yaxis2 = list(title = "Price"),       # Y-axis for the price chart
    yaxis3 = list(title = "Share"),       # Y-axis for the share chart
    yaxis4 = list(title = "Threshold"),  # Y-axis for the threshold chart
    yaxis5 = list(title = "USDC Amount"), # Y-axis for the USDC reserve chart
    yaxis6 = list(title = "Supplied")     # Y-axis for the supplied chart
  )
